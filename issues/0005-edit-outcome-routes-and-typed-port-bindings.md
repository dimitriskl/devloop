...
Add structured graph editing beyond the Primary Path. For every outcome declared by a Step Contract, allow routing to an existing step, creating a new branch step, inserting a step between a source and destination, or terminating intentionally. Support loops and branch-local ordering. Keep the interaction keyboard-operable and provide a live graph preview rather than requiring drag-and-drop.
