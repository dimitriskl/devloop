...
Extend the v2 runtime slice so any Step Instance can be paused, resumed, retried, or revisited without phase-specific state. Checkpoint the exact workflow cursor by Step Instance ID and optional Issue. Support changes-requested outcomes from either Security Review or Final Review: return to Development, create a new Development attempt, then execute both reviews and QA again according to the configured graph.
...
- [ ] A run can pause and resume at every Step Instance in the four-step reference workflow using persisted generic state.
