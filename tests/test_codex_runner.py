...
class RolePromptIdentityTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            with mock.patch.object(
                ...
                for attempt_id, session_id in (
                    ("attempt/identity/with/shared/prefix/one", "session-one"),
                    ("attempt/identity/with/shared/prefix/two", "session-two"),
                ):
                    runner.run_role(
                        role="reviewer",
                        issue=issue,
    ...
    def test_blocker_resolution_role_artifacts_fit_legacy_windows_path_limit(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            issue_path = root / "0001.md"
            issue_path.write_text("# Blocker resolution\n", encoding="utf-8")

            base_log_root = (root / ".loop.logs").resolve()
            padding_length = 130 - len(str(base_log_root)) - 1
            self.assertGreater(padding_length, 0)
            log_root = root / ("x" * padding_length) / ".loop.logs"
            self.assertEqual(len(str(log_root.resolve())), 130)
            ...
            def write_with_legacy_windows_limit(
                path: Path,
                text: str,
                *args: object,
                **kwargs: object,
            ...
            with mock.patch.object(
                ...
                Path,
                "write_text",
                new=write_with_legacy_windows_limit,
            ):
                result = runner.run_role(
                    role="coder",
                    issue=Issue("0001", "Blocker resolution", issue_path, False),
                    pass_number=1,
                    attempt_label="blocker-resolution-1",
                    step_instance_id="9c30a1c0-57b4-4cf6-8b6d-a568dac11e01",
                    step_display_name="Development",
                    step_attempt_id="cab8df7b-a06c-477e-9818-40f10ecd7ba3",
                )

            artifact_paths = [
                *runner.log_root.glob("*.prompt.md"),
                *runner.log_root.glob("*.stdout.jsonl"),
                *runner.log_root.glob("*.stderr.txt"),
            ]
            captured_message_path[0].write_text(
                '{"status":"PASS","summary":"implemented"}',
                encoding="utf-8",
            )
            recovered_legacy_passes = recover_role_passes(
                runner.log_root,
                Issue("0001", "Blocker resolution", issue_path, False),
            )

        self.assertEqual(result.status, "PASS")
        self.assertEqual(len(artifact_paths), 3)
        self.assertTrue(captured_message_path)
        self.assertTrue(
            all(len(str(path.resolve())) < 260 for path in artifact_paths)
        )
        self.assertLess(len(str(captured_message_path[0].resolve())), 260)
        self.assertEqual(recovered_legacy_passes, [])
